...
function AdminPanel() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      setSession(session);
      if (session) fetchApplicants(); else setLoading(false);
    });
  }, []);
  ...
  const handleLogin = async (e) => {
    ...
    if (error) { alert(error.message); setLoading(false); } else { window.location.reload(); }
  };
  ...
  if (!supabase) return <div style={{padding: 50, textAlign:'center'}}>⚠️ Critical Error: Missing Supabase Keys.</div>;
      ...
      <div className="page-content-wrapper">
        <div className="glass-box" style={{ maxWidth: 400, width: '100%', textAlign: 'center' }}>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
            ...
            <button type="submit" className="primary-btn" disabled={loading} style={{ justifyContent: 'center' }}>{loading ? "Authenticating..." : "Login →"}</button>
          </form>
        </div>
      </div>
    );
            ...
            {applicants.map(app => (
              <div key={app.id} className="glass-box" style={{ marginTop: 0, padding: 24, cursor: 'pointer', display: 'flex', flexDirection: 'column' }} onClick={() => setExpandedId(expandedId === app.id ? null : app.id)}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 8 }}>
                  <div><strong style={{ fontSize: 18, display: 'block' }}>{app.name}</strong><span style={{ fontSize: 12, color: '#ff5a3c', textTransform: 'uppercase', letterSpacing: 1 }}>{app.role}</span></div>
                ...
                {expandedId === app.id && (
                  <div style={{ marginTop: 15, paddingTop: 15, borderTop: '1px solid var(--border)', fontSize: 14, lineHeight: 1.6, animation: 'fadeIn 0.3s ease' }}>
                    <p><strong>Phone:</strong> {app.phone}</p><p><strong>College:</strong> {app.college} ({app.year})</p>
                    <div style={{ marginTop: 15 }}><strong style={{ display:'block', marginBottom: 5, fontSize: 12, opacity: 0.5, textTransform: 'uppercase' }}>Role Specifics</strong>
                      <div style={{ background: 'rgba(0,0,0,0.2)', padding: 12, borderRadius: 8, fontSize: 13 }}>
                        {app.role_answers && Object.entries(app.role_answers).map(([k, v]) => (<div key={k} style={{ marginBottom: 6 }}><span style={{ opacity: 0.6 }}>{k}:</span> <br/>{Array.isArray(v) ? v.join(", ") : v}</div>))}
                      </div>
                    </div>
                    <div style={{ marginTop: 15 }}><strong style={{ display:'block', marginBottom: 5, fontSize: 12, opacity: 0.5, textTransform: 'uppercase' }}>Common</strong>
                      {app.common_answers && Object.entries(app.common_answers).map(([k, v]) => (<div key={k} style={{ marginBottom: 6 }}><span style={{ opacity: 0.6 }}>{k}:</span> <br/>{v}</div>))}
                    </div>
                  </div>
                )}
              </div>
    ...
    <div className="fixed-header">
      <div className="header-content">
        ...
        <div className="header-right">
          ...
          <button className="icon-btn" onClick={toggleTheme} aria-label="Toggle Theme">{theme === "dark" ? <SunIcon /> : <MoonIcon />}</button>
          <button className="mobile-menu-btn" onClick={() => setIsMenuOpen(!isMenuOpen)}>{isMenuOpen ? <CloseIcon /> : <MenuIcon />}</button>
        </div>
      </div>
      {isMenuOpen && <div className="mobile-dropdown"><button className="mobile-link" onClick={() => handleNav("landing")}>Home</button><button className="mobile-link" onClick={() => handleNav("about")}>About Us</button><button className="mobile-link" onClick={() => handleNav("team")}>Our Team</button><button className="mobile-link" onClick={() => handleNav("who")}>Who is this for?</button></div>}
    </div>
  );
}
  ...
  return (
    <footer className="site-footer">
      <div className="footer-content-wrapper">
        <div className="footer-col"><div className="logo-text" style={{ color: "#ff5a3c", marginBottom: 20, fontSize: 24 }}>AAKAAR.io</div><div className="contact-item"><span style={{ color: "var(--text-muted)", fontSize: 12 }}>Email</span><div style={{ marginTop: 4 }}>support@aakario.com</div></div><div className="contact-item"><span style={{ color: "var(--text-muted)", fontSize: 12 }}>Phone</span><div style={{ marginTop: 4 }}>+91 8280669173</div></div><div style={{ marginTop: 30, display: 'flex', gap: 15 }}><a href="#" style={{ color: 'var(--text-sub)', fontSize: 13 }}>Privacy Policy</a><a href="#" style={{ color: 'var(--text-sub)', fontSize: 13 }}>Terms of Service</a></div></div>
        <div className="footer-col map-col"><div className="map-frame"><iframe src="https://maps.google.com/maps?q=KIIT+University+Bhubaneswar&t=&z=13&ie=UTF8&iwloc=&output=embed" width="100%" height="100%" style={{ border: 0 }} allowFullScreen="" loading="lazy"></iframe></div><div style={{ marginTop: 10, fontSize: 12, color: "var(--text-muted)", textAlign: "right" }}>KIIT University, Patia, Bhubaneswar, Odisha 751024</div></div>
      </div>
  ...
  return (
    <div className="page-content-wrapper">
      <div className="content-box fade-in" style={{ textAlign: "left", maxWidth: 640 }}>
        ...
        <div className="glass-box"><h3 className="section-title">What We Do</h3><p style={{ color: "var(--text-sub)", lineHeight: 1.7, marginBottom: 40 }}>BuildAI is an AI-powered platform designed to democratize software creation. We are building tools that allow anyone—regardless of their technical background—to generate, deploy, and scale web applications using natural language.</p><h3 className="section-title">Our Vision</h3><p style={{ color: "var(--text-sub)", lineHeight: 1.7 }}>We envision a world where software development is accessible to everyone.</p></div>
      </div>
    </div>
  );
}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', justifyContent: 'center' }}>
           {TEAM_MEMBERS.map((m, i) => (
             <div key={i} className="role-card" style={{ textAlign: "center", padding: "40px 20px" }}>
               <div style={{ width: 100, height: 100, background: "rgba(255,90,60,0.1)", borderRadius: "50%", margin: "0 auto 24px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, color: "#ff5a3c", border: "1px solid rgba(255,90,60,0.2)" }}>{m.name.charAt(0)}</div>
  ...
  return (
    <div className="page-content-wrapper">
      <div className="content-box landing-box">
        ...
        <div style={rv(3)} className="role-grid">{ROLES.map((r) => (<div key={r.id} className="role-card"><div className="role-head">{r.icon} {r.label}</div><div className="role-desc">{r.desc}</div></div>))}</div>
        <div style={rv(4)}><button onClick={onStart} className="primary-btn">Apply now <span style={{ fontSize: 18 }}>→</span></button><p className="micro-text">Takes ~5 minutes · No résumés</p></div>
        <div style={{ ...rv(5), marginTop: 60, paddingTop: 40, paddingBottom: 80, borderTop: "1px solid var(--border)" }}><Newsletter /></div>
      </div>
    </div>
  );
}
...
function FormPage({ onBack }) {
  ...
  useEffect(() => { if (localStorage.getItem("aakaar_submitted")) { setHasSubmitted(true); setStep(4); } }, []);
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await supabase.from('applications').insert([{ name: basics.name, email: basics.email, phone: `${basics.countryCode} ${basics.phone}`, college: basics.college, year: basics.year, role: selectedRole, role_answers: roleAnswers, common_answers: commonAnswers }]);
    setIsSubmitting(false);
    if (error) { error.code === '23505' ? alert("Already applied.") : alert("Error submitting."); } else { localStorage.setItem("aakaar_submitted", "true"); setHasSubmitted(true); setStep(4); }
  };

  if (hasSubmitted && step !== 4) return (<div className="page-content-wrapper"><div className="content-box form-box" style={{ textAlign: 'center' }}><div className="success-badge">✓</div><h2 className="section-title">Already Applied!</h2><button onClick={onBack} className="secondary-btn" style={{marginTop:30}}>← Back Home</button></div></div>);
    ...
    <div className="page-content-wrapper">
      <div className="content-box form-box">
        ...
        <div className="form-content-pad">
          {step === 0 && <div className="fade-in"><h2 className="section-title">About you</h2><input type="text" style={{display:'none'}} value={basics.honeypot} onChange={e=>setBasics({...basics, honeypot:e.target.value})} /><div className="field-group"><Label>Name</Label><input type="text" value={basics.name} onChange={e=>setBasics({...basics, name:e.target.value})} style={inputStyle(!!errors.name)} /><ErrorMsg msg={errors.name}/></div><div className="field-group"><Label>Year</Label><SelectField value={basics.year} onChange={v=>setBasics({...basics, year:v})} options={YEAR_OPTIONS} hasError={!!errors.year} /><ErrorMsg msg={errors.year}/></div><div className="field-group"><Label>College</Label><input type="text" value={basics.college} onChange={e=>setBasics({...basics, college:e.target.value})} style={inputStyle(!!errors.college)} /><ErrorMsg msg={errors.college}/></div><div className="field-group"><Label>Phone</Label><div style={{display:'flex', gap:10}}><div style={{width:90}}><SelectField value={basics.countryCode} onChange={v=>setBasics({...basics, countryCode:v})} options={COUNTRY_CODES}/></div><input type="tel" value={basics.phone} onChange={e=>{const v=e.target.value.replace(/\D/g,""); if(v.length<=10)setBasics({...basics, phone:v})}} style={{...inputStyle(!!errors.phone), flex:1}} /></div><ErrorMsg msg={errors.phone}/></div><div className="field-group"><Label>Email</Label><input type="email" value={basics.email} onChange={e=>setBasics({...basics, email:e.target.value})} style={inputStyle(!!errors.email)} /><ErrorMsg msg={errors.email}/></div></div>}
          ...
          {/* NAVIGATION BUTTONS - ALIGNMENT FIXED */}
          {step < 4 && <div className="nav-row">
            <button onClick={()=>step===0?onBack():back()} className="secondary-btn">← Back</button>
            <button onClick={()=>step===3?handleSubmit():next()} className="primary-btn" disabled={isSubmitting}>
              {isSubmitting ? "Sending..." : step===3 ? "Submit" : "Continue"}
            </button>
          </div>}
        </div>
      </div>
...
export default function App() {
  ...
  if (urlPath === '/admin') return (<><GlobalStyles theme={theme} /><CursorBlob /><AdminPanel /></>);

  return (
    <>
      <GlobalStyles theme={theme} />
      {showRickRoll && (<div style={{ position: "fixed", inset: 0, zIndex: 10000, background: "black", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}><h1 style={{ color: "#ff5a3c", fontSize: "40px", marginBottom: "30px", fontFamily: "'Playfair Display', serif", textAlign: 'center', padding: '0 20px' }}>Light mode is for pussies XD</h1><iframe width="800" height="450" src="https://www.youtube.com/embed/dQw4w9WgXcQ?autoplay=1" title="Rick Roll" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen style={{ maxWidth: '100vw', maxHeight: '50vh' }}></iframe><button onClick={() => setShowRickRoll(false)} style={{ marginTop: "30px", padding: "10px 20px", background: "#333", color: "white", border: "none", borderRadius: "5px", cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>I understand, take me back</button></div>)}
      ...
      <CursorBlob />
      <FixedHeader onNavigate={setPage} theme={theme} toggleTheme={() => setShowRickRoll(true)} />
      {page === "landing" ? <Landing onStart={() => setPage("form")} /> : page === "form" ? <FormPage onBack={() => setPage("landing")} /> : page === "about" ? <About /> : page === "team" ? <Team /> : <WhoIsThisFor />}
      <Footer />
    </>
  );
}
...
function GlobalStyles({ theme }) {
  return (
    <style>{`
        ...
        :root { --bg: #0c0c0e; --text: #ffffff; --text-sub: rgba(255,255,255,0.5); --text-muted: rgba(255,255,255,0.3); --border: rgba(255,255,255,0.1); --glass-bg: rgba(255,255,255,0.03); --glass-border: rgba(255,255,255,0.08); --input-bg: rgba(255,255,255,0.06); --accent: #ff5a3c; --nav-text: rgba(255,255,255,0.6); --nav-hover: #ffffff; --header-bg: rgba(12, 12, 14, 0.85); --grid-color: rgba(255, 255, 255, 0.03); }
        body.light { --bg: #f2f2f7; --text: #1a1a1e; --text-sub: rgba(26, 26, 30, 0.6); --text-muted: rgba(26, 26, 30, 0.4); --border: rgba(26, 26, 30, 0.1); --glass-bg: rgba(255, 255, 255, 0.7); --glass-border: rgba(26, 26, 30, 0.06); --input-bg: #ffffff; --accent: #ff5a3c; --nav-text: rgba(26, 26, 30, 0.6); --nav-hover: #1a1a1e; --header-bg: rgba(242, 242, 247, 0.85); --grid-color: rgba(0, 0, 0, 0.05); }
        html, body, #root { width: 100%; min-height: 100vh; margin: 0; padding: 0; background: var(--bg); color: var(--text); overflow-x: hidden; font-family: 'Inter', sans-serif; transition: background 0.3s, color 0.3s; background-image: linear-gradient(var(--grid-color) 1px, transparent 1px), linear-gradient(90deg, var(--grid-color) 1px, transparent 1px); background-size: 30px 30px; background-position: center top; background-attachment: fixed; }
        ...
        /* NEW SECONDARY BUTTON STYLE - FIXES ALIGNMENT */
        .secondary-btn { background: transparent; border: 1px solid var(--border); color: var(--text); padding: 14px 32px; border-radius: 8px; font-size: 15px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; transition: all 0.2s; font-family: 'Inter', sans-serif; }
        .secondary-btn:hover { background: rgba(255,255,255,0.05); border-color: var(--text-sub); }
